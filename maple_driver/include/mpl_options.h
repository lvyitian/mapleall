...
class MplOptions {
 public:
  MplOptions(MemPool *mp) : optMp(mp), meOptions(*mp), mpl2mplOptions(*mp), cgOptions(*mp) {}
 ...
 private:
  ...
  void UpdateExes(const std::string &args);
 ...
 public:
  MemPool *optMp;
  MeOption meOptions;           // mplme's option processing
  Options mpl2mplOptions;       // mpl2mpl's option processing
  CGOptions cgOptions;          // mplcg's option processing
};
}  // namespace maple
