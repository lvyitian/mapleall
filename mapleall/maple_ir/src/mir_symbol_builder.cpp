...
MIRSymbol *MIRSymbolBuilder::GetSymbol(TyIdx tyIdx, GStrIdx strIdx, MIRSymKind mClass, MIRStorageClass sClass,
  ...
  if (sameType) {
    if (st->GetStorageClass() == sClass && st->sKind == mClass) {
      return st;
    }
    return nullptr;
  }
  ASSERT(mClass == st->sKind,
...
MIRSymbol *MIRSymbolBuilder::CreateSymbol(TyIdx tyIdx, GStrIdx strIdx, MIRSymKind mClass, MIRStorageClass sClass,
  ...
  MIRSymbol *st =
      func != nullptr ? func->symTab->CreateSymbol(scpID) : GlobalTables::GetGsymTable().CreateSymbol(scpID);
  ...
  st->tyIdx = tyIdx;
  if (func) {
    (void)func->symTab->AddToStringSymbolMap(st);
  } else {
    (void)GlobalTables::GetGsymTable().AddToStringSymbolMap(st);
  }
  return st;
