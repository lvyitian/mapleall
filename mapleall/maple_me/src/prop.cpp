  ...
  switch (opop) {
    case OP_cvt: {
      ...
      if (opnd0->meOp == kMeOpConst) {
        ...
        if (tocvt) {
          return irMap->CreateConstMeExpr(cvtmeexpr->primType, tocvt);
        }
      }
      if (opnd0->op == OP_cvt) {
        ...
        if (maple::GetPrimTypeSize(cvtopnd0->primType) >= maple::GetPrimTypeSize(cvtopnd0->GetOpnd(0)->primType)) {
          if ((maple::IsPrimitiveInteger(cvtopnd0->primType) && maple::IsPrimitiveInteger(cvtopnd0->GetOpnd(0)->primType)) ||
            ...
            return irMap->CreateMeExprTypeCvt(cvtmeexpr->primType, cvtopnd0->opndType, cvtopnd0->GetOpnd(0));
          }
        }
    ...
    case OP_depositbits: {
      ...
      return irMap->CreateConstMeExpr(opmeexpr->primType, resconst);
    }
    case OP_ne:
    case OP_eq:
    case OP_lt:
        ...
        if (opmeopnd0->op == OP_select) {
          ...
          if (opnd01->meOp == kMeOpConst && IsPrimitivePureScalar(opnd01->primType) && opnd02->meOp == kMeOpConst &&
            ...
            if (canbereplaced) {
              ...
              MeExpr *newopnd01 = irMap->HashMeExpr(&xnewopnd01);
              ConstMeExpr xnewopnd02(-1, constopnd02, PTY_u1);
              MeExpr *newopnd02 = irMap->HashMeExpr(&xnewopnd02);
              if (needswapopnd) {
  ...
  switch (meOp) {
    ...
    case kMeOpOp: {
      ...
      if (subproped) {
        ...
        MeExpr *splfmeexpr = SimplifyMeExpr(&newmeexpr);
