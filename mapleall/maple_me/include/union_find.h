...
class UnionFind {
 ...
 public:
  UnionFind(MemPool *mp) : ufAlloc(mp), num(0), id(ufAlloc.Adapter()), sz(ufAlloc.Adapter()) {}

  UnionFind(MemPool *mp, uint32 siz)
    : ufAlloc(mp), num(siz), id(siz, 0, ufAlloc.Adapter()), sz(siz, 0, ufAlloc.Adapter()) {
    Reinit();
  }
...
// for the root id's, the sum of their size should be population size
    ...
    CHECK_FATAL(sum == num, "Something wrong in UnionFind");
#endif
  }
...
#endif  // end MAPLE_ME_INCLUDE_UNION_FIND_H
