...
#include "riscv64_opnd.def"
#define DEFINE_MOP(op,a2,a3,a4,a5,a6,a7,a8,a9,aa,ab,ac) {op,a2,a3,a4,a5,a6,a7,a8,a9,aa,ab,ac},
const Riscv64MD Riscv64CG::kMd[kMopLast] = {
#include "riscv64_md.def"
};
#undef DEFINE_MOP

bool Riscv64CG::IsExclusiveFunc(MIRFunction *mirFunc) {
 ...
 */
void Riscv64CG::FindOrCreateRepresentiveSym(vector<uint64_t> &bitmapWords, uint32_t rcHeader, string name) {
  ...
  if (iter == key2pattern.end()) {
    ...
    emitter_->Emit("\t.long ");  // Riscv64-specific. Generate a 64-bit value.
    ...
    for (auto bitmapWord : bitmapWords) {
      ...
      emitter_->Emit("\t.quad ");  // Riscv64-specific. Generate a 64-bit value.
...
string Riscv64CG::FindGCTIBPatternName(const string &name) {
...
void Riscv64CG::GenerateObjectMaps(BECommon &becommon) {
  if (!DoItQuietly()) {
  ...
  for (auto tyid : mirModule->classList) {
    ...
    CHECK_FATAL(dynamic_cast<MIRStructType *>(ty) != nullptr, "ty isn't MIRStructType* in Riscv64CG::GenerateObjectMaps");
...
void Riscv64Insn::CheckOpnd(Operand *opnd, OpndProp *prop) {
  Riscv64OpndProp *mopd = static_cast<Riscv64OpndProp *>(prop);
...
bool Riscv64Insn::IsDefinition() const {
  // check if we are seeing ldp or not
  CG_ASSERT(!Riscv64CG::kMd[mop_].GetOperand(1) || !Riscv64CG::kMd[mop_].GetOperand(1)->IsRegDef(), "");
  if (Riscv64CG::kMd[mop_].GetOperand(0) == nullptr) {
    return false;
  }
  return Riscv64CG::kMd[mop_].GetOperand(0)->IsRegDef();
}

bool Riscv64Insn::IsDestRegAlsoSrcReg() const {
  Riscv64OpndProp *prop0 = static_cast<Riscv64OpndProp *>(Riscv64CG::kMd[mop_].GetOperand(0));
...
bool Riscv64Insn::IsConversionInstruction() const {
  return ((Riscv64CG::kMd[mop_].properties_ & ISCONVERSION) != 0);
}

bool Riscv64Insn::IsConditionalSet() const {
  return ((Riscv64CG::kMd[mop_].properties_ & ISCONDSET) != 0);
}

RegOperand *Riscv64Insn::GetOperandDefined() {
