 ...
 */
bool Riscv64GlobalOpt::NoOverlap(Insn *targetInsn, int opndidx, bool isBg) {
 ...
 */
bool Riscv64GlobalOpt::FindAThroughPath(Insn *insnStart, Insn *insnEnd) {
...
bool Riscv64GlobalOpt::TraverseNext(BB *curBb, BB *target, std::set<BB *> *traversedBb) {
  if (curBb == target) {
    return true;
  }
 ...
 */
bool Riscv64GlobalOpt::HasMultiGen(Insn *newInsn) {
 ...
 * mop yy, x1
 */
void Riscv64GlobalOpt::ForwardPropOfRegister() {
 ...
 *
 */
void Riscv64GlobalOpt::InsertDUUDForDestOperand(Insn *newInsn, short newIndex, unsigned short newProp,
...
bool Riscv64GlobalOpt::TraverseDomNext(BB *start, BB *end, std::set<BB *> *middleSet, std::set<uint32_t> *traversed) {
  if (start == end) {
    return false;
  }
 ...
 */
bool Riscv64GlobalOpt::IsPartialDominate(Insn *startInsn, Insn *endInsn, std::set<Insn *> *middleInsnSet) {
...
bool Riscv64GlobalOpt::OnlyOneDefine(Insn *targetInsn) {
  for (auto use : *(targetInsn->uses[0])) {
    Insn *useInsn = use.GetInsn();
...
bool Riscv64GlobalOpt::OpndOnlyDefInCurBB(Insn *targetInsn, int opndidx) {
  BB *curBb = targetInsn->bb;
 ...
 */
void Riscv64GlobalOpt::CleanDDForDestOperand(Insn *insn, unsigned short index) {
 ...
 */
void Riscv64GlobalOpt::BackPropOfRegister() {
  bool secondTime = false;
  std::set<BB *> modifiedBb;
  do {
    FOR_ALL_BB(bb, cgfunc) {
 ...
 * def_insn is mov
 */
void Riscv64GlobalOpt::ReplaceInsnSrcOperand(Insn *mInsn, short i, unsigned short prop, Operand *newOpnd,
                                             Insn *defInsn) {
  CHECK_FATAL(i >= 0, "must not be call insn");
...
void Riscv64GlobalOpt::DeleteRedundantUxt() {
// FIXME
#if 0
  FOR_ALL_BB(bb, cgfunc) {
    if (bb->unreachable) {
 ...
 *   mov reg2, reg1    ----|
 */
int Riscv64GlobalOpt::GetMaximumValidBit(Insn *insn, short udidx, std::set<Insn *> &insnChecked) const {
...
int Riscv64GlobalOpt::GetInsnValidBit(Insn *insn) {
...
bool Riscv64GlobalOpt::OpndDefByOne(Insn *insn, int useidx) const {
...
bool Riscv64GlobalOpt::OpndDefByZero(Insn *insn, int useidx) const {
  CG_ASSERT(insn->GetOperand(useidx) && insn->GetOperand(useidx)->IsRegister(), "the used Operand must be Register");
  // Zero Register don't need be defined
  if (insn->GetOperand(useidx)->IsZeroRegister()) {
    return true;
...
bool Riscv64GlobalOpt::InsnDefOne(Insn *insn) {
...
bool Riscv64GlobalOpt::InsnDefZero(Insn *insn) {
  CG_ASSERT(insn, "insn must not be null");
  MOperator defMop = insn->GetMachineOpcode();
  switch (defMop) {
    case MOP_xmovri64: {
...
bool Riscv64GlobalOpt::OpndDefByOneOrZero(Insn *insn, int useidx) const {
  if (insn->GetOperand(useidx)->IsZeroRegister()) {
    return true;
  }
  CG_ASSERT(insn->defs[useidx], "Operand must be defined before used");
...
bool Riscv64GlobalOpt::InsnDefOneOrZero(Insn *insn) {
  CG_ASSERT(insn, "insn must not be null");
  MOperator defMop = insn->GetMachineOpcode();
  switch (defMop) {
    case MOP_xslt:
...
bool Riscv64GlobalOpt::OpndOnlyUsedInCurBB(Insn *insn, short opndidx, bool isDest) {
  if (isDest) {
...
bool Riscv64GlobalOpt::RedefPointIsExist(Insn *insn, short opndidx) {
  return insn->redefine[opndidx] != nullptr;
}

bool Riscv64GlobalOpt::IsSameReg(Operand *firstOpnd, Operand *secondOpnd) {
