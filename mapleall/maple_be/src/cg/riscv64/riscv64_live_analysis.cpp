...
void Riscv64LiveAnalysis::InitEhDefine(BB *bb) {
  Riscv64CGFunc *aarchcgfunc = static_cast<Riscv64CGFunc *>(cgfunc_);

  // Insert MOP_pseudo_eh_def_x R1.
  RegOperand *regopnd = aarchcgfunc->GetOrCreatePhysicalRegisterOperand(R1, 64, kRegTyInt);
  Insn *pseudoInsn = cgfunc_->cg->BuildInstruction<Riscv64Insn>(MOP_pseudo_eh_def_x, regopnd);
  ...
  pseudoInsn = cgfunc_->cg->BuildInstruction<Riscv64Insn>(MOP_pseudo_eh_def_x, regopnd);
  bb->InsertInsnBegin(pseudoInsn);
}

regno_t Riscv64LiveAnalysis::UpdateRegNum(RegOperand *opnd) {
  ...
  Riscv64RegOperand *aopnd = dynamic_cast<Riscv64RegOperand *>(opnd);
    ...
    if (type == kVecSingle) {
      switch (aopnd->GetSimdVectorPosition()) {
        ...
        case 3:
          ...
          break;
        default:
          CG_ASSERT(0, "Riscv64LiveAnalysis::UpdateRegNum Error");
      }
    } else {
      // VEC_DOUBLE
      switch (aopnd->GetSimdVectorPosition()) {
...
void Riscv64LiveAnalysis::CollectLiveInfo(BB *bb, Operand *opnd, bool isDef, bool isUse) {
...
BB* Riscv64LiveAnalysis::CreateSubBB(BB *bb) {
  subBBId++;
...
bool Riscv64LiveAnalysis::CanInsnThrow(Insn *insn) {
  if (insn->CanThrow()) {
    if (insn->IsMemAccess()) {
...
void Riscv64LiveAnalysis::GetInsnDefUse(BB *bb, Insn *insn) {
  if (insn->IsCall()) {
    for (uint32 i = 0; i < 8; i++) {
      Operand *phyopnd = static_cast<Riscv64CGFunc *>(cgfunc_)->GetOrCreatePhysicalRegisterOperand(
        (Riscv64reg_t)(R0 + i), 64, kRegTyInt);
      CollectLiveInfo(bb, phyopnd, true, false);

      phyopnd = static_cast<Riscv64CGFunc *>(cgfunc_)->GetOrCreatePhysicalRegisterOperand((Riscv64reg_t)(V0 + i), 64,
                                                                                          kRegTyFloat);
      CollectLiveInfo(bb, phyopnd, true, false);
    }
  }
...
void Riscv64LiveAnalysis::BreakBBIntoSubBB(BB *bb) {
 ...
 */
void Riscv64LiveAnalysis::GetBBDefUse(BB *bb) {
  if (bb->IsEmpty()) {
    return;
  }
...
bool Riscv64LiveAnalysis::CleanupBBIgnoreReg(uint32 reg) {
...
void Riscv64LiveAnalysis::FinalizeLiveAnalysis() {
