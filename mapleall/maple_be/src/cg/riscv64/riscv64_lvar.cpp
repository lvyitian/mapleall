...
void Riscv64OptLocalRef::DoOptLocalRef() {
  if (CfgHasCycle() == true) {
...
void Riscv64OptLocalRef::PrintBbTraceList(string str, bool more) {
 ...
 */
bool Riscv64OptLocalRef::DetectCycle() {
  BB *bb = nullptr;
  bool childPushed;
  while (dfs_bbs.empty() == false) {
    ...
    bb = dfs_bbs.top();
    dfs_bbs.pop();
    CHECK_FATAL(bb != nullptr, "bb is null in Riscv64OptLocalRef::DFS");
...
bool Riscv64OptLocalRef::CfgHasCycle() {
...
bool Riscv64OptLocalRef::IsLdStLocalRefVar(Operand *src) {
  ...
  Riscv64OfstOperand *imm = memopnd->GetOffsetImmediate();
  if (imm == nullptr) {
    return false;
  }
 ...
 */
void Riscv64OptLocalRef::ModifyParamValue(Insn *insn, uint32 reg, uint32 val) {
 ...
 */
uint32 Riscv64OptLocalRef::GetParamValue(Insn *insn, uint32 reg) {
  // Assuming at this point the parameters are generated in order and in bb.
  // Therefore x1, x2 will always not be the first insn in bb.
  for (Insn *prev = insn->prev; prev; prev = prev->prev) {
    Operand *dst = prev->GetOperand(0);
 ...
 */
void Riscv64OptLocalRef::HandleCleanupBB(BB *bb) {
 ...
 */
bool Riscv64OptLocalRef::IsRcStackCall(Insn *insn) {
  if (insn->IsCall()) {
    ...
    if (callName) {
      ...
      if (name == GetIntrinsicFuncName(INTRN_MCCInitializeLocalStackRef)) {
        ...
        int skipSlot = static_cast<Riscv64cleancallInsn *>(insn)->ref_skipindex;
        if (skipSlot != -1) {
          giveup = true;
 ...
 */
void Riscv64OptLocalRef::RecordLdStPair(Insn *insn) {
  ...
  Riscv64MemOperand *mem = static_cast<Riscv64MemOperand *>(src);
 ...
 */
bool Riscv64OptLocalRef::RecordXzrInit(Insn *insn) {
  if (static_cast<RegOperand *>(insn->GetOperand(0))->IsZeroRegister()) {
    ...
    Riscv64MemOperand *mem = static_cast<Riscv64MemOperand *>(src1);
    Riscv64OfstOperand *imm = mem->GetOffsetImmediate();
    if (imm != nullptr) {
      int32 val = imm->GetOffsetValue();
      CG_ASSERT((val & 0x7) == 0, "Ld/St pair imm offset not 8 aligned");
...
bool Riscv64OptLocalRef::IsLdStPair(Insn *insn) {
  switch (insn->mop_) {
    case MOP_wldaxp:
    case MOP_wstlxp:
      CG_ASSERT(0, "Riscv64OptLocalRef IsLdStPair not handle 32 bit");
    case MOP_xldaxp:
    case MOP_xstlxp:
      return true;
  }
 ...
 */
bool Riscv64OptLocalRef::CanGetMemOpndImm(Operand *src, int32 &val) {
...
bool Riscv64OptLocalRef::ImmOffUsedInXzrInit(int32 imm) {
...
bool Riscv64OptLocalRef::ImmOffUsedInLdStPair(int32 imm) {
  MapleSet<int32>::iterator it;
 ...
 */
void Riscv64OptLocalRef::RemoveImmUsedInSkip() {
 ...
 * condition and do not modify these localrefvar.
 */
void Riscv64OptLocalRef::RemoveImmUsedInLdSt() {
  for (uint32 i = 0; i < cgfunc->NumBBs(); i++) {
    Iinfo *insninfo = insn_info_in_bb[i];
    Iinfo *previnfo = nullptr;
    while (insninfo != nullptr) {
 ...
 */
void Riscv64OptLocalRef::OptLRInit() {
  ...
  FOR_ALL_BB(bb, cgfunc) {
    FOR_BB_INSNS(insn, bb) {
      ...
      if (imm >= max_imm) {
        other_opnd.insert(static_cast<Riscv64MemOperand *>(src));
        continue;
      }
      if (ImmOffUsedInXzrInit(imm) == true) {
        continue;
 ...
 */
void Riscv64OptLocalRef::OptLRFini() {
  FOR_ALL_BB(bb, cgfunc) {
    FOR_BB_INSNS(insn, bb) {
      insn->id = 0;
    }
 ...
 */
bool Riscv64OptLocalRef::CanRenameConflictSlot(int32 imm) {
 ...
 */
void Riscv64OptLocalRef::AssignLocalRefForTrace() {
  vector<int32> immMap;
  vector<bool> seen;
...
void Riscv64OptLocalRef::DoOptimize() {
  visited_bbs.resize(cgfunc->NumBBs());
  for (uint32 i = 0; i < cgfunc->NumBBs(); i++) {
    visited_bbs[i] = false;
  }
 ...
 */
void Riscv64OptLocalRef::FinalizeLocRef() {
  ...
  for (auto it = other_opnd.begin(); it != other_opnd.end(); it++) {
    Riscv64MemOperand *src = *it;
    Riscv64OfstOperand *ofstopnd = src->GetOffsetImmediate();
    int32 offset = ofstopnd->GetOffsetValue();
    if (offset <= max_assigned) {
      continue;
    }
    Riscv64OfstOperand *offsetOpnd = cgfunc->memPool->New<Riscv64OfstOperand>(offset - diff, 64);
...
/* Patch the MRT calls.
 */
void Riscv64OptLocalRef::FixMrtCall() {
  ...
  static_cast<Riscv64MemLayout *>(cgfunc->memlayout)->Reflocals().size = val * SIZEOFPTR;
}
