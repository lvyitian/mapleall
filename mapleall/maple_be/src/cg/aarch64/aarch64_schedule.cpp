...
void AArch64Schedule::MemoryAccessPairOpt(BB *bb) {
}
...
void AArch64Schedule::RegPressureScheduling(const BB *bb, std::vector<DepNode*> &nodes) {
 ...
 */
void AArch64Schedule::DoSchedule() {
...
void AArch64Schedule::DumpDepGraph(const std::vector<DepNode*> &nodes) const {
  for (auto node : nodes) {
...
void AArch64Schedule::GenerateDot(const BB *bb, const std::vector<DepNode*> &nodes) const {
...
void AArch64Schedule::ListScheduling(bool beforeRA) {
  ...
  mad = g->mad;
  depAnalysis = mp->New<AArch64DepAnalysis>(cgfunc, mp, beforeRA);
