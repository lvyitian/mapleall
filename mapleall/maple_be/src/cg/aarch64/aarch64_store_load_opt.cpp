...
bool AArch64StoreLoadOpt::CanReachEndBBFromBB(BB *bb, BB *endBB, set<BB *> &traversedBBSet) {
  if (bb == endBB) {
    return true;
  }
...
bool AArch64StoreLoadOpt::IsLiveInAllPathBB(BB *startBB, BB *endBB, MapleSet<BB *> &setBb,
 ...
 */
bool AArch64StoreLoadOpt::IsLiveAtInsn(Insn *defInsn, int index, Insn *startInsn, Insn *endInsn) {
  if (defInsn->IsStore()) {
...
void AArch64StoreLoadOpt::InsertDUUD(const DataInsnInfo &defInsnInfo, const DataInsnInfo &useInsnInfo) {
...
void AArch64StoreLoadOpt::RemovDUUD(Insn *defInsn, Insn *useInsn) {
 ...
 */
void AArch64StoreLoadOpt::DoLoadToMoveTransfer(Insn *stInsn, short stIdx, Insn *ldInsn, short ldIdx,
  ...
  if (version & (kOptVersionStpLive | kOptVersionStrLive)) {
    ...
    if (movInsn == nullptr) {
      ...
      newMovInsn = cgfunc->cg->BuildInstruction<AArch64Insn>(newMop, vregOpnd, srcRegOpnd);
  ...
  if (cgfunc->cg->GenerateVerboseAsm() && cgfunc->cg->GenerateTestInfo()) {
    ...
    if (version & (kOptVersionStpLive | kOptVersionStpDie)) {
      ...
      Insn *commentInsn = cgfunc->cg->BuildInstruction<AArch64Insn>(MOP_comment, a64cgfunc->CreateCommentOperand(str1));
      ...
      commentInsn = cgfunc->cg->BuildInstruction<AArch64Insn>(MOP_comment, a64cgfunc->CreateCommentOperand(str2));
...
/* Transfer: store wzr, [MEM]
   ...
   Params:
     ...
     ldIdx: index of source memory operand of the load insn.
 */
void AArch64StoreLoadOpt::DoLoadZeroToMoveTransfer(Insn *stInsn, short stIdx, Insn *ldInsn, short ldIdx,
    ...
    FOR_BB_INSNS(insn, bb) {
      ...
      if (insn->IsStore()) {
        ...
        switch (insn->GetMachineOpcode()) {
          ...
          case MOP_dstp: {
            for (short i = 0; i < 2; i++) {
              // Check if memory address has a use.
              if (insn->uses[i]) {
                RegOperand *regOpnd = static_cast<RegOperand *>(insn->GetOpnd(i));
                CG_ASSERT(regOpnd != nullptr, "null ptr check");
                if (regOpnd->IsZeroRegister()) {
                  MapleSet<DataInsnInfo, DataInsnInfoCmp2> &setUses = *(insn->uses[i]);
                  for (itSet2 = setUses.begin(); itSet2 != setUses.end(); itSet2 = itSetNext2) {
                    ...
                    if (SCHDDUMP) {
                      LogInfo::MapleLogger() << "Do store-load optimization, stp -- wzr/xzr version: ";
                    ...
                    DoLoadZeroToMoveTransfer(insn, i, ldrInsn, ldrInfo.GetIndex(), kOptVersionStpZero);
                  }
                ...
                if (insn->defs[i]->size() != 1 || insn->bb->is_cleanup) {
                  continue;
                }

                DataInsnInfo insnInfo = *(insn->defs[i]->begin());

                Insn *defInsn = insnInfo.GetInsn();
                int opndIdx = insnInfo.GetIndex();
                int index = opndIdx + insnInfo.GetProperty() & DataInsnInfo::kSecondMemAddr;
                ...
                for (itSet2 = setUses.begin(); itSet2 != setUses.end(); itSet2 = itSetNext2) {
                  ...
                  if (IsLiveAtInsn(defInsn, index, insn, ldrInsn)) {
                     ...
                     */
                    if (0 && SCHDDUMP) {
                      LogInfo::MapleLogger() << "Do store-load optimization 1: stp version";
                      LogInfo::MapleLogger() << cgfunc->GetName() << std::endl;
                      LogInfo::MapleLogger() << "Store insn: ";
                      insn->dump();
                      LogInfo::MapleLogger() << "Load insn: ";
                    ...
                    DoLoadToMoveTransfer(insn, i, ldrInsn, ldrInfo.GetIndex(), kOptVersionStpLive, insnInfo);
                  } else {
                    // Do the optimization.
                    /* Optimize: store x100, [MEM]
                     ...
                     */
                    if (SCHDDUMP) {
                      LogInfo::MapleLogger() << "Do store-load optimization 2: stp version";
                      LogInfo::MapleLogger() << cgfunc->GetName() << std::endl;
                      LogInfo::MapleLogger() << "Store insn: ";
                      insn->dump();
                      LogInfo::MapleLogger() << "Load insn: ";
                    ...
                    DoLoadToMoveTransfer(insn, i, ldrInsn, ldrInfo.GetIndex(), kOptVersionStpDie, insnInfo);
                  }
                }
                if (setUses.empty()) {
                  insn->uses[i] = nullptr;
