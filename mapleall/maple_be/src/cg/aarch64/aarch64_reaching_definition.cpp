 ...
 */
bool AArch64ReachingDefinition::MayHasThrow(const Insn *headInsn, const Insn *tailInsn) {
...
void AArch64ReachingDefinition::InitStartGen() {
...

    {
      ...
      if (!sym->IsPreg()) {
        ...
        AArch64SymbolAlloc *firstsymloc =
          static_cast<AArch64SymbolAlloc *>(cgfunc->memlayout->sym_alloc_table[firstsym->GetStIndex()]);
        ...
        AArch64MemOperand *memOpnd = cgfunc->memPool->New<AArch64MemOperand>(RFP, stoffset, firststksize * 8);
        MOperator mop = firststksize <= 4 ? MOP_pseudo_param_store_w : MOP_pseudo_param_store_x;
        Insn *pseudoInsn = cgfunc->cg->BuildInstruction<AArch64Insn>(mop, memOpnd);
        bb->InsertInsnBegin(pseudoInsn);
        pseudoInsns.insert(pseudoInsn);
      }
    }
  ...
  for (int i = 0; i < a64cgfunc->refCount; i++) {
    AArch64MemOperand *memOpnd = cgfunc->memPool->New<AArch64MemOperand>(RFP, a64cgfunc->beginOffset + i * 8, 64);
    Insn *pseudoInsn = cgfunc->cg->BuildInstruction<AArch64Insn>(MOP_pseudo_ref_init_x, memOpnd);
...
void AArch64ReachingDefinition::InitEhDefine(BB *bb) {
  ...
  Insn *pseudoInsn = cgfunc->cg->BuildInstruction<AArch64Insn>(MOP_pseudo_eh_def_x, regopnd);
  bb->InsertInsnBegin(pseudoInsn);
  pseudoInsns.insert(pseudoInsn);

  // Insert MOP_pseudo_eh_def_x R0.
  regopnd = aarchcgfunc->GetOrCreatePhysicalRegisterOperand(R0, 64, kRegTyInt);
  pseudoInsn = cgfunc->cg->BuildInstruction<AArch64Insn>(MOP_pseudo_eh_def_x, regopnd);
  bb->InsertInsnBegin(pseudoInsn);
  pseudoInsns.insert(pseudoInsn);
}
...
void AArch64ReachingDefinition::AddRetPseudoInsn(BB *bb) {
  AArch64reg_t regno = static_cast<AArch64CGFunc *>(cgfunc)->GetReturnRegisterNumber();
  ...
  if (regno == R0) {
    RegOperand *regopnd =
      static_cast<AArch64CGFunc *>(cgfunc)->GetOrCreatePhysicalRegisterOperand(regno, 64, kRegTyInt);
    retInsn = cgfunc->cg->BuildInstruction<AArch64Insn>(MOP_pseudo_ret_int, regopnd);
  } else {
    CG_ASSERT(regno == V0, "CG internal error. Return value should be R0 or V0.");
    RegOperand *regopnd =
      static_cast<AArch64CGFunc *>(cgfunc)->GetOrCreatePhysicalRegisterOperand(regno, 64, kRegTyFloat);
    retInsn = cgfunc->cg->BuildInstruction<AArch64Insn>(MOP_pseudo_ret_float, regopnd);
  }
 ...
 */
void AArch64ReachingDefinition::KillAllCallerSavedRegs(BB *bb, Insn *callInsn) {
  AArch64CGFunc *a64cgfunc = static_cast<AArch64CGFunc *>(cgfunc);
  RegOperand *phyopnd;
  int i;
  ...
  for (i = R1; i <= R18; i++) {
    phyopnd = a64cgfunc->GetOrCreatePhysicalRegisterOperand((AArch64reg_t)i, 64, kRegTyInt);
    bb->gen.erase(DataAnalysisInfo(kInfoReg, phyopnd));
  }
  ...
  for (i = V16; i <= V31; i++) {
    phyopnd = a64cgfunc->GetOrCreatePhysicalRegisterOperand((AArch64reg_t)i, 64, kRegTyFloat);
    bb->gen.erase(DataAnalysisInfo(kInfoReg, phyopnd));
  }
}

void AArch64ReachingDefinition::DirtyAllNonStackMem(BB *bb) {
  return;
}
 ...
 */
void AArch64ReachingDefinition::KillAllCallerSavedRegsOnBBIn(BB *bb, Insn *callInsn) {
...
/* For aarch64, R0/V0 as the return register.
 */
void AArch64ReachingDefinition::GenReturnValue(BB *bb, Insn *insn) {
  map<DataAnalysisInfo, DataInsnInfo, DataAnalysisInfoCmp>::iterator itGen;

  AArch64CGFunc *a64cgfunc = static_cast<AArch64CGFunc *>(cgfunc);
  RegOperand *phyopnd = nullptr;
  if (insn->ret_type == Insn::kRegInt) {
    phyopnd = a64cgfunc->GetOrCreatePhysicalRegisterOperand((AArch64reg_t)R0, 64, kRegTyInt);
    ...
    phyopnd = a64cgfunc->GetOrCreatePhysicalRegisterOperand((AArch64reg_t)V0, 64, kRegTyFloat);
    bb->gen.insert(pair<DataAnalysisInfo, DataInsnInfo>(DataAnalysisInfo(kInfoReg, phyopnd), DataInsnInfo(insn, -1)));
  }
}
...
/* For aarch64, R0/V0 as the return register.
 */
void AArch64ReachingDefinition::GenReturnValueOnBBIn(BB *bb, Insn *insn) {
  ...
  if (insn->ret_type == Insn::kRegInt) {
    RegOperand *phyopnd = a64cgfunc->GetOrCreatePhysicalRegisterOperand((AArch64reg_t)R0, 64, kRegTyInt);
...
void AArch64ReachingDefinition::InitKillGen(BB *bb, int mode) {
  if (bb->IsEmpty()) {
    return;
  }
      ...
      if (opnd->IsList()) {
        ...
        if (base == nullptr) {
          ...
          if (memopnd->GetAddrMode() == AArch64MemOperand::kAddrModeBOi &&
            ...
            DefineRegister(bb, base, insn, i, DataInsnInfo::kBaseReg);
          }
          ...
          if (IsFrameReg(base)) {
            if (index) {
              ...
              // if is stp, deinfine next mem offset
              switch (insn->GetMachineOpcode()) {
                case MOP_wstp:
                case MOP_sstp: {
                  Operand *nextOpnd = static_cast<AArch64MemOperand *>(opnd)->Clone(mp);
                  AArch64MemOperand *nextMemOpnd = static_cast<AArch64MemOperand *>(nextOpnd);
                  ...
                  DefineMem(bb, nextMemOpnd, insn, i, DataInsnInfo::kSecondMemAddr);
                  break;
                }
                case MOP_xstp:
                case MOP_dstp: {
 ...
 */
void AArch64ReachingDefinition::GenerateUseDef(BB *bb, int mode) {
  ...
  for (Insn *insn = bb->firstinsn; insn; insn = insn->next) {
    ...
    for (int i = 0; i < Insn::kMaxOperandNum; i++) {
      ...
      if (opnd->IsMemoryAccessOperand()) {
        ...
        AArch64MemOperand *memopnd = static_cast<AArch64MemOperand *>(opnd);
        if (memopnd->GetAddrMode() == AArch64MemOperand::kAddrModeLiteral) {
          continue;
        }
        ...
        if (opnd->IsRegister() || opnd->IsMemoryAccessOperand()) {
          ...
          if (insn->IsLoad()) {
            ...
            if (memopnd->GetAddrMode() == AArch64MemOperand::kAddrModeBOi &&
                (memopnd->IsPostIndexed() || memopnd->IsPreIndexed())) {
              // Base operand has changed.
              DefineRegisterOnBBIn(bb, memopnd->GetBaseRegister(), insn, i, DataInsnInfo::kBaseReg);
            }
            ...
            switch (insn->GetMachineOpcode()) {
              ...
              case MOP_xldpsw: {
                ...
                CG_ASSERT(aarchNextOfstOpnd, "aarchNextOfstOpnd should not be nullptr.");

                int32 offsetVal = aarchNextOfstOpnd->GetOffsetValue();
                aarchNextOfstOpnd->SetOffsetValue(offsetVal + 4);
                nextMemOpnd->SetOffsetImmediate(aarchNextOfstOpnd);

                itMap = bb->in.find(DataAnalysisInfo(kInfoMem, nextMemOpnd));
                if (itMap != bb->in.end()) {
                  insn->defs[i + 1] = mp->New<MapleSet<DataInsnInfo, DataInsnInfoCmp>>(rdalloc.Adapter());
                  for (auto s : itMap->second) {
                    insn->defs[i]->insert(s);
                  }
                } else {
          ...
          itMap = bb->in.find(DataAnalysisInfo(kInfoReg, rflagOpnd));
          if (itMap != bb->in.end()) {
            insn->defs[i] = mp->New<MapleSet<DataInsnInfo, DataInsnInfoCmp>>(rdalloc.Adapter());
            for (auto s : itMap->second) {
              insn->defs[i]->insert(s);
        ...
        if (opnd->IsMemoryAccessOperand()) {
          ...
          if (base == nullptr) {
            ...
            if (memopnd->GetAddrMode() == AArch64MemOperand::kAddrModeBOi &&
                (memopnd->IsPostIndexed() || memopnd->IsPreIndexed())) {
              // Base operand has changed.
              DefineRegisterOnBBIn(bb, base, insn, i, DataInsnInfo::kBaseReg);
            }
            if (!(mode & RD_MEMANALYSIS)) {
              continue;
            }
            ...
            if (IsFrameReg(base)) {
              if (index) {
                ...
                // if is stp/ldp, deinfine next mem offset
                switch (insn->GetMachineOpcode()) {
                  case MOP_wstp:
                  case MOP_sstp: {
                    Operand *nextOpnd = static_cast<AArch64MemOperand *>(opnd)->Clone(mp);
                    ...
                    aarchNextOfstOpnd->SetOffsetValue(offsetVal + 4);
                    nextMemOpnd->SetOffsetImmediate(aarchNextOfstOpnd);
                    DefineMemOnBBIn(bb, nextMemOpnd, insn, i, DataInsnInfo::kSecondMemAddr);
                    break;
                  }
                  case MOP_xstp:
                  case MOP_dstp: {
...
void AArch64ReachingDefinition::InsertUseToDef(DataInsnInfo &insnInfo, Insn *useInsn, short index, short userProperty) {
...
void AArch64ReachingDefinition::GenerateDefUse(BB *bb) {
  FOR_BB_INSNS(insn, bb) {
    if (!insn->IsMachineInstruction()) {
      continue;
    }
...
void AArch64ReachingDefinition::DumpUDChain(BB *bb) {
          ...
          if (i <= memOpndIdx) {
            ...
            switch (property) {
              ...
              case DataInsnInfo::kBaseReg: {
                ...
                AArch64MemOperand *memOpnd = static_cast<AArch64MemOperand *>(insn->opnds[memOpndIdx]);
...
void AArch64ReachingDefinition::DumpDUChain(BB *bb) {
...
void AArch64ReachingDefinition::DumpWAWDependence(BB *bb) {
  LogInfo::MapleLogger() << "\n    WAW dependence: " << endl;
 ...
 */
void AArch64ReachingDefinition::RemoveDUUDForInsn(Insn *insn) {
 ...
 */
void AArch64ReachingDefinition::InsertDUUDForSrcOperand(Insn *newInsn, short newIndex, unsigned short newProp,
 ...
 */
void AArch64ReachingDefinition::InsertDUUDForDestOperand(Insn *newInsn, short newIndex, unsigned short newProp,
                                                         Insn *refInsn, short refIndex, unsigned short refProp,
...
/* Uses: Replace one of the insn operand, with maintaining data flow info.
   ...
   CALLPARAM)
 */
void AArch64ReachingDefinition::ReplaceInsnSrcOperand(Insn *insn, short index, unsigned short prop, Operand *newOpnd,
                                                      Insn *refInsn, short refIndex, unsigned short refProp) {
  MapleSet<DataInsnInfo, DataInsnInfoCmp2>::iterator itSet2;
  ...
  switch (prop & DataInsnInfo::kAnyIndex) {
    ...
    case DataInsnInfo::kIndexReg: {
      ...
      MemOperand *memOpnd =
        static_cast<MemOperand *>(static_cast<MemOperand *>(insn->opnds[index])->Clone(cgfunc->memPool));
      CG_ASSERT(memOpnd != nullptr, "memOpnd is null in AArch64CGFunc::ReplaceInsnSrcOperand");
      insn->SetOperand(index, memOpnd);
 ...
 */
void AArch64ReachingDefinition::ReplaceInsnDestOperand(Insn *insn, short index, unsigned short prop, Operand *newOpnd,
                                                       Insn *refInsn, short refIndex, unsigned short refProp,
  ...
  switch (prop & DataInsnInfo::kAnyIndex) {
    ...
    case DataInsnInfo::kBaseReg: {
      ...
      MemOperand *memOpnd =
        static_cast<MemOperand *>(static_cast<MemOperand *>(insn->opnds[index])->Clone(cgfunc->memPool));
      CG_ASSERT(memOpnd != nullptr, "memOpnd is null in AArch64CGFunc::ReplaceInsnDestOperand");
      insn->SetOperand(index, memOpnd);
