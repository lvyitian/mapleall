...
#include "aarch64/aarch64_ra_opt.h"
#include "mir_lower.h"
#include "cg_assert.h"
#include "securec.h"
...
void GraphColorRegAllocator::CheckLiveRangeConflicts(const LiveRange *lr) const {
#ifdef USE_VEC
  if (lr->sconflict.size() != lr->numBconflicts) {
    LogInfo::MapleLogger() << "Error: PrintLiveRange inconsistent conflict num\n";
    CG_ASSERT(0, "Error: PrintLiveRange inconsistent conflict num");
  }
  for (uint32 i = 0; i < regBuckets; i++) {
    uint64 chunk = lr->bconflict[i];
    for (uint64 bit = 0; bit < (sizeof(uint64) * CHAR_BIT); bit++) {
      if (chunk & (1LL << bit)) {
        regno_t newno = i * sizeof(uint64) * CHAR_BIT + bit;
        if (FIND_NOT_IN(lr->sconflict, newno)) {
          LogInfo::MapleLogger() << "Error: PrintLiveRange inconsistent conflict member\n";
          CG_ASSERT(0, "Error: PrintLiveRange inconsistent conflict member");
        }
      }
    }
  }
...
void GraphColorRegAllocator::PrintLiveBbSet(const LiveRange *lr, const string str) const {
#ifdef USE_VEC
  if (lr->smember.size() != lr->numBmembers) {
    LogInfo::MapleLogger() << "Error: PrintLiveBbSet inconsistent size\n";
    CG_ASSERT(0, "Error: PrintLiveBbSet inconsistent size");
  }
  LogInfo::MapleLogger() << str << "(" << lr->smember.size() << "): ";
  for (uint32 i = 0; i < cgfunc_->NumBBs(); i++) {
    if (IS_BIT_ARR_ELEM_SET(lr->bmember, i)) {
      if (FIND_NOT_IN(lr->smember, bbVec[i])) {
        LogInfo::MapleLogger() << "Error: PrintLiveBbSet inconsistent member\n";
        CG_ASSERT(0, "Error: PrintLiveBbSet inconsistent member");
      }
      LogInfo::MapleLogger() << i << " ";
    }
  }
  LogInfo::MapleLogger() << "\n";
  // To match the ordering of bit by using bbid order.
  set<BB *, SetBBCmpFunc> result;
  for (auto bb : lr->smember) {
    result.insert(bb);
  }
  LogInfo::MapleLogger() << str << "(" << lr->smember.size() << "): ";
  for (auto bb : result) {
...
void GraphColorRegAllocator::PrintLiveRange(const LiveRange *lr, const string str) const {
  ...
  LogInfo::MapleLogger() << "\n";
#ifdef USE_VEC
  PrintLiveBbSet(lr, "\tlive_bb");
#endif  // USE_VEC
  PrintLiveBbBit(lr);
#ifdef USE_VEC
  CheckLiveRangeConflicts(lr);
  LogInfo::MapleLogger() << "\tinterfere(" << lr->sconflict.size() << "): ";
  for (auto newno : lr->sconflict) {
    LogInfo::MapleLogger() << newno << ",";
  }
  LogInfo::MapleLogger() << "\n";
#endif  // USE_VEC
...
regno_t GraphColorRegAllocator::StartingIntCallerReg() {
  return (R9 - R0);
}

regno_t GraphColorRegAllocator::EndingIntCallerReg() {
#ifdef RESERVED_REGS
  return (R15 - R0);  // 16,17 are used besides ra.
#elif                 // RESERVED_REGS
  return (R17 - R0);
#endif                // RESERVED_REGS
}

regno_t GraphColorRegAllocator::StartingIntCalleeReg() {
...
regno_t GraphColorRegAllocator::EndingIntCalleeReg() {
  return MaxIntPhysRegNum();
}

regno_t GraphColorRegAllocator::StartingFpCallerReg() {
  return (V16 - V0);
}

regno_t GraphColorRegAllocator::EndingFpCallerReg() {
  return MaxFloatPhysRegNum();
}

regno_t GraphColorRegAllocator::StartingFpCalleeReg() {
  return (V8 - V0);
}

regno_t GraphColorRegAllocator::EndingFpCalleeReg() {
  return (V15 - V0);
}

void GraphColorRegAllocator::GetAvailablePRregs(RegType regty, uint32 &begin, uint32 &end, bool isCaller) {
  // ==== int regs ====
  ...
  switch (regty) {
    ...
    case kRegTyInt:
      if (isCaller) {
        begin = StartingIntCallerReg();
        end = EndingIntCallerReg();
      } else {
        begin = StartingIntCalleeReg();
        end = EndingIntCalleeReg();
      }
      break;
    case kRegTyFloat:
      if (isCaller) {
        begin = StartingFpCallerReg();
        end = EndingFpCallerReg();
      } else {
        begin = StartingFpCalleeReg();
        end = EndingFpCalleeReg();
      }
      break;
    default:
      CG_ASSERT(false, "NYI");
...
void GraphColorRegAllocator::InitFreeRegPool() {
  ...
  intCallerRegSet.insert(R8 - R0);
  intNum++;
  for (uint32 i = 0; i <= (kMaxParamNum - 1); i++) {
    intCallerRegSet.insert(i);
  }
  intNum += kMaxParamNum;
  intCallerRegSet.insert(R18 - R0);
  intNum++;
  for (uint32 i = 0; i <= (kMaxParamNum - 1); i++) {
    fpCallerRegSet.insert(i);
  }
...
#endif  // REMOVE_INSN
  bool isMovk = insn->GetMachineOpcode() == MOP_wmovkri16 || insn->GetMachineOpcode() == MOP_xmovkri16;
  if (CreateLiveRange(regno, insn->bb, isDef, isMovk, insn->id, true)) {
...
void GraphColorRegAllocator::ComputeLiveRangesUpdateIfInsnIsCall(Insn *insn) {
  if (insn->IsCall()) {
    ...
    if (opnd1->IsList()) {
      AArch64ListOperand *srcopnds = static_cast<AArch64ListOperand *>(opnd1);
...
#if defined(USE_VEC) && defined(CONFLICT_CHECK)
    if (lr->numBmembers != lr->smember.size()) {
      LogInfo::MapleLogger() << "Err: bit size member " << lr->regno << endl;
      PrintLiveRange(lr, "LR-SIZE-DIFF");
    }
    if (lr->numBconflicts != lr->sconflict.size()) {
      LogInfo::MapleLogger() << "Err: bit size conflict " << lr->regno << endl;
      PrintLiveRange(lr, "LR-SIZE-DIFF");
    }
#endif  // USE_VEC && CONFLICT_CHECK
  }
}
...
void GraphColorRegAllocator::ComputeLiveOut(BB *bb) {
  ...
  if (bb->HasCall() == false && bb->succs.size() == 1) {
    AArch64reg_t regno = static_cast<AArch64CGFunc *>(cgfunc_)->GetReturnRegisterNumber();
...
#ifdef USE_VEC
    return (lr->smember.size() == 1) && (!lr->isNonLocal);
#else   // USE_VEC
...
void GraphColorRegAllocator::CopyInterference() {
#if !defined(COPY_VEC) && defined(USE_VEC)
  set<BB *, SetBBCmpFunc> result;
  result.clear();
  for (auto bb : lr1->smember) {
    if (FIND_IN(lr2->smember, bb)) {
      result.insert(bb);
    }
    if (result.size() > 1) {
      break;
  ...
  if (result.size() == 0) {
    ...
    auto lu1 = lr1->luMap.find((*it)->id);
    auto lu2 = lr2->luMap.find((*it)->id);
    if (lu1 != lr1->luMap.end() && lu2 != lr2->luMap.end() &&
      ...
      lr1->sconflict.insert(lr2->regno);
      lr2->sconflict.insert(lr1->regno);
    }
  } else {
    // interfere
    lr1->sconflict.insert(lr2->regno);
...
#ifdef USE_LRA
#ifdef USE_VEC
    if (lrVec[i]->smember.size() == 1)
#else   // USE_VEC
    if (lrVec[i]->numBmembers == 1)
#endif  // USE_VEC
    {
      continue;
    }
...
void GraphColorRegAllocator::BuildInterferenceGraphCopyVec() {
#ifdef COPY_VEC
  // Copy bit vector info into std set
  std::vector<LiveRange *>::iterator it;
  for (it = intLrVec.begin(); it != intLrVec.end(); it++) {
    LiveRange *lr = *it;
    for (uint32 i = 0; i < regBuckets; i++) {
      uint64 chunk = lr->bconflict[i];

      uint64 size = sizeof(uint64) * CHAR_BIT;
      for (uint64 bit = 0; bit < size; bit++) {
        if (chunk & (1LL << bit)) {
          regno_t regno = i * sizeof(uint64) * CHAR_BIT + bit;
          lr->sconflict.insert(regno);
        }
      }
    }
  }
  for (it = fpLrVec.begin(); it != fpLrVec.end(); it++) {
    LiveRange *lr = *it;
    for (uint32 i = 0; i < regBuckets; i++) {
      uint64 chunk = lr->bconflict[i];
...
void GraphColorRegAllocator::BuildInterferenceGraphErrorCheck() {
#if defined(USE_VEC) && defined(CONFLICT_CHECK)
  for (uint32_t i = 0; i < lrVec.size(); i++) {
    if (lrVec[i] == nullptr) {
      continue;
    }
    for (auto confReg : lrVec[i]->sconflict) {
      if (IS_BIT_ARR_ELEM_SET(lrVec[i]->bconflict, confReg) == false) {
        LogInfo::MapleLogger() << "Err: set conflict " << confReg << " in LR reg " << lrVec[i]->regno << "\n";
        PrintLiveRange(lrVec[i], "LR-SCONFLICT-DIFFER");
      }
    }
    FOREACH_REG_ARR_ELEM(lrVec[i]->bconflict, confReg)
    if (FIND_NOT_IN(lrVec[i]->sconflict, confReg)) {
      LogInfo::MapleLogger() << "Err: bit conflict " << confReg << " in LR reg " << lrVec[i]->regno << "\n";
      PrintLiveRange(lrVec[i], "LR-BCONFLICT-DIFFER");
    }
...
#ifdef COPY_VEC
  BuildInterferenceGraphCopyVec();
#else  // If copying, there is no need to check
#if defined(USE_VEC) && defined(CONFLICT_CHECK)
  BuildInterferenceGraphErrorCheck();
#endif  // USE_VEC && CONFLICT_CHECK
#endif  // COPY_VEC
...
#endif  // USE_LRA
#ifdef OPTIMIZE_FOR_PROLOG
    if ((lr->numDefs <= 1 && lr->numUses <= 1 && lr->numCall > 0) &&
...
#endif  // OPTIMIZE_FOR_PROLOG
#ifdef USE_VEC
    if (HaveAvailableColor(lr, lr->sconflict.size() + lr->pregveto.size() + lr->forbidden.size()))
#else   // USE_VEC
...
bool GraphColorRegAllocator::ShouldUseCallee(LiveRange *lr, MapleSet<regno_t> &calleeUsed,
  ...
  if (FIND_IN(calleeUsed, lr->assigned)) {
    return true;
  } else if (AArch64Abi::IsCalleeSavedReg((AArch64reg_t)(lr->assigned)) && calleeUsed.size() % 2 != 0) {
    return true;
...
bool GraphColorRegAllocator::AssignColorToLr(LiveRange *lr, bool isDelayed) {
  ...
  if (HaveAvailableColor(lr, lr->numForbidden + lr->numPregveto)) {
    ...
    bool isCalleeReg = AArch64Abi::IsCalleeSavedReg((AArch64reg_t)(lr->assigned));
    if (isCalleeReg) {
      if (lr->regtype == kRegTyInt) {
...
void GraphColorRegAllocator::SplitCopyVecMember(LiveRange *lr) {
#ifdef USE_VEC
  lr->smember.clear();  // Use bmember to update smember
  for (uint32 i = 0; i < bbBuckets; i++) {
    uint64 chunk = lr->bmember[i];
    uint64 size = sizeof(uint64) * CHAR_BIT;
    for (uint64 bit = 0; bit < size; bit++) {
      if (chunk & (1LL << bit)) {
        BB *bb = bbVec[i * sizeof(uint64) * CHAR_BIT + bit];
        lr->smember.insert(bb);
      }
    }
  }
...
#ifdef USE_VEC
  for (auto bbit = lr->smember.begin(); bbit != lr->smember.end(); bbit++) {
    BB *bb = *bbit;
#else   // USE_VEC
  FOREACH_BB_ARR_ELEM(lr->bmember, bbid)
  BB *bb = bbVec[bbid];
...
void GraphColorRegAllocator::ComputeBbForNewSplit(LiveRange *newLr, LiveRange *origLr) {
  ...
  set<CgfuncLoops *> defInLoop;
#ifdef USE_VEC
  for (auto bbit = newLr->smember.rbegin(); bbit != newLr->smember.rend(); bbit++)
#else   // USE_VEC
...
#ifdef USE_VEC
  ClearLrBbFlags(origLr->smember);
  for (auto bb : origLr->smember)
#else   // USE_VEC
  set<BB *, SortedBBCmpFunc> smember;
...
#ifdef USE_VEC
  auto lrIt = FIND(oldLr->smember, bb);
  if (lrIt != oldLr->smember.end()) {
    oldLr->smember.erase(lrIt);
  }
#endif  // USE_VEC
...
#ifdef USE_VEC
  if (lr->splitLr || lr->smember.size() == 1)
#else   // USE_VEC
...
#ifdef USE_VEC
    BB *bb = bbVec[bbid];
    auto bbIt = FIND(newLr->smember, bb);
    if (bbIt != newLr->smember.end()) {
      newLr->smember.erase(bbIt);
    }
    origLr->smember.insert(bb);
#endif  // USE_VEC
    UNSET_MEMBER_BIT_ARR_ELEM(newLr, bbid);
...
#ifdef USE_VEC
  for (auto bb : newLr->smember) {
    for (auto regno : lr->sconflict) {
      LiveRange *conf_lr = lrVec[regno];
      if (IS_BIT_ARR_ELEM_SET(conf_lr->bmember, bb->id)) {
        ...
        newLr->sconflict.insert(regno);
        SET_CONFLICT_BIT_ARR_ELEM(newLr, regno);
      } else if (conf_lr->splitLr && FIND_IN(conf_lr->splitLr->smember, bb)) {
        newLr->sconflict.insert(regno);
        SET_CONFLICT_BIT_ARR_ELEM(newLr, regno);
      }
    }
...
#if defined(USE_VEC) && defined(CONFLICT_CHECK)
  if (origLr->numBconflicts != origLr->sconflict.size()) {
    LogInfo::MapleLogger() << "Err: lr bit size conflict " << origLr->regno << endl;
    PrintLiveRange(origLr, "LR-SIZE-DIFF");
  }
  if (newLr->numBconflicts != newLr->sconflict.size()) {
    LogInfo::MapleLogger() << "Err: newLr bit size conflict " << origLr->regno << endl;
    PrintLiveRange(origLr, "LR-SIZE-DIFF");
  }
#endif  // USE_VEC && CONFLICT_CHECK
...
void GraphColorRegAllocator::HandleLocalRaDebug(regno_t regno, LocalRegAllocator *localRa, bool isInt) {
  ...
  for (uint32 regno = 0; regno < RZR; regno++) {
    if (regs & (1LL << regno)) {
...
void GraphColorRegAllocator::LocalRaRegSetEraseReg(LocalRegAllocator *localRa, regno_t regno) {
  bool isInt;
  if (AArch64isa::IsGPRegister(static_cast<AArch64reg_t>(regno))) {
    isInt = true;
  } else {
    isInt = false;
  }
...
void GraphColorRegAllocator::SpillOperandForSpillPost(Insn *insn, Operand *opnd, RegOperand *phyopnd, uint32 spillIdx,
  ...
  if (lastInsn) {
    for (auto tbb : insn->bb->succs) {
      Insn *newLd = cg->BuildInstruction<AArch64Insn>(a64cgfunc->PickLdInsn(spillMem->GetSize(), stype), phyopnd, spillMem);
      newLd->SetSpillOp();
    ...
    if (isOutOfRange) {
      insn->bb->InsertInsnAfter(insn->next, ldInsn);
    } else {
...
Insn *GraphColorRegAllocator::SpillOperand(Insn *insn, Operand *opnd, bool isDef, RegOperand *phyopnd,
  ...
  if (isDef) {
    ...
    if (isOutOfRange || (insn->next && insn->next->GetMachineOpcode() == MOP_clinit_tail)) {
      insn->bb->InsertInsnAfter(insn->next, spillDefInsn);
    } else {
      insn->bb->InsertInsnAfter(insn, spillDefInsn);
    }
  }
  if (!isDef || ((insn->GetMachineOpcode() == MOP_xmovkri16 || insn->GetMachineOpcode() == MOP_wmovkri16) && isDef)) {
    ...
    spillUseInsn = cg->BuildInstruction<AArch64Insn>(a64cgfunc->PickLdInsn(regsize, stype), phyopnd, memopnd);
...
void GraphColorRegAllocator::CollectCannotUseReg(set<regno_t> &cannotUseReg, LiveRange *lr, Insn *insn) {
  ...
  FOREACH_REG_ARR_ELEM(lr->bconflict, regno)
    ...
    if ((conflictLr->assigned > 0) && IS_BIT_ARR_ELEM_SET(conflictLr->bmember, insn->bb->id)) {
      if (!AArch64Abi::IsCalleeSavedReg((AArch64reg_t)conflictLr->assigned) && conflictLr->numCall) {
        continue;
      }
  ...
  if (bbInfo) {
    ...
    if (regIt != bbInfo->regMap.end()) {
      RegOperand *phyOpnd = static_cast<AArch64CGFunc *>(cgfunc_)->GetOrCreatePhysicalRegisterOperand(
          (AArch64reg_t)(regIt->second), regOpnd->GetSize(), regtype);
      return phyOpnd;
    } else {
      if (GCRA_DUMP) {
      ...
      if ((insn->GetMachineOpcode() == MOP_clinit_tail) ||
        ...
        bool isCalleeReg = AArch64Abi::IsCalleeSavedReg((AArch64reg_t)(spillReg));
        if (isCalleeReg) {
          if (regtype == kRegTyInt) {
      ...
      RegOperand *phyOpnd = static_cast<AArch64CGFunc *>(cgfunc_)->GetOrCreatePhysicalRegisterOperand(
          (AArch64reg_t)(spillReg), regOpnd->GetSize(), regtype);
...
RegOperand *GraphColorRegAllocator::GetReplaceOpnd(Insn *insn, Operand *opnd, uint32 &spillIdx, uint64 &usedRegMask,
  ...
  RegOperand *phyOpnd = static_cast<AArch64CGFunc *>(cgfunc_)->GetOrCreatePhysicalRegisterOperand(
      (AArch64reg_t)(regno), opnd->GetSize(), regtype);
  if (GCRA_DUMP) {
