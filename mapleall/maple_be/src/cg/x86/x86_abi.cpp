...
// described by the X86-64 ABI
static X86_64_ArgumentClass MergeClasses(X86_64_ArgumentClass class0, X86_64_ArgumentClass class1) {
  if (class0 == class1) {
    return class0;
  }
  if (class0 == X86_64_NoClass) {
    return class1;
  }
  if (class1 == X86_64_NoClass) {
    return class0;
  }
  if (class0 == X86_64_MemoryClass || class1 == X86_64_MemoryClass) {
    return X86_64_MemoryClass;
  }
  if (class0 == X86_64_IntegerClass || class1 == X86_64_IntegerClass) {
    return X86_64_IntegerClass;
  }
  if (class0 == X86_64_X87Class || class1 == X86_64_X87Class || class0 == X86_64_X87upClass ||
      class1 == X86_64_X87upClass) {
    return X86_64_MemoryClass;
  }
  return X86_64_SseClass;
}
...
static int32 ClassifyAggregate(BECommon &be, MIRType *ty, X86_64_ArgumentClass classes[2], size_t classesLength) {
  ...
  for (i = 0; i < sizeofty_in_dwords; i++) {
    classes[i] = X86_64_NoClass;
  }
  if (ty->typeKind != kTypeStruct && ty->typeKind != kTypeClass && ty->typeKind != kTypeArray) {
    switch (ty->GetPrimType()) {
      ...
      case PTY_u64:
      case PTY_i64:
        classes[0] = X86_64_IntegerClass;
        return 1;
      case PTY_f32:
      case PTY_c64:
      case PTY_f64:
        classes[0] = X86_64_SseClass;
        return 1;
      case PTY_c128:
        classes[0] = X86_64_SseClass;
        classes[1] = X86_64_SseClass;
        return 2;
      default:
        CG_ASSERT(false, "");
    }
    ...
    X86_64_ArgumentClass subclasses[2];
    int32 subnumregs;
    if (ty->typeKind == kTypeStruct) {
      ...
      for (uint32 f = 0; f < sty->fields.size(); f++) {
        MIRType *fieldty = globaltable.GetTypeFromTyIdx(sty->fields[f].second.first);
      ...
      if (subnumregs < sizeofty_in_dwords) {
        for (i = 1; i < sizeofty_in_dwords; i++)
          if (classes[i] == X86_64_NoClass) {
            classes[i] = classes[i - 1];
          }
      }
    } else if (ty->typeKind == kTypeClass) {
      MIRClassType *sty = static_cast<MIRClassType *>(ty);
      uint32 fld_bofst = 0;  // offset of field in bits within immediate struct
      uint64 alloced_size = 0;
      MIRType *prnt_ty = globaltable.GetTypeFromTyIdx(sty->parentTyIdx);
      subnumregs = ClassifyAggregate(be, prnt_ty, subclasses);
      if (subnumregs == 0) {
        return 0;
      }
      // determine fld_bofst for this field
      uint32 prnt_size = be.type_size_table[prnt_ty->_ty_idx.idx];
      CG_ASSERT(prnt_size != 0, "");
      uint8 prnt_align = be.type_align_table[prnt_ty->_ty_idx.idx];
      CG_ASSERT(prnt_align != 0, "");
      // pad alloced_size according to the field alignment
      alloced_size = RoundUp(alloced_size, prnt_align);
      fld_bofst = alloced_size * 8;
      alloced_size += prnt_size;
      // merge subclasses into classes
      int32 indx = fld_bofst >> 6;  // index into the struct in doublewords
      for (i = 0; i < subnumregs; i++) {
        classes[i + indx] = MergeClasses(classes[i + indx], subclasses[i]);
      ...
      for (uint32 f = 0; f < sty->fields.size(); f++) {
        MIRType *fieldty = globaltable.GetTypeFromTyIdx(sty->fields[f].second.first);
        subnumregs = ClassifyAggregate(be, fieldty, subclasses);
        if (subnumregs == 0) {
          return 0;
        }
        // determine fld_bofst for this field
    ...
    } else {  // ty->typeKind == TYPE_ARRAY
      MIRArrayType *aty = static_cast<MIRArrayType *>(ty);
      subnumregs = ClassifyAggregate(be, globaltable.GetTypeFromTyIdx(aty->eTyIdx), subclasses);
      if (subnumregs == 0) {
        return 0;
      }
      for (i = 0; i < sizeofty_in_dwords; i++) {
    ...
    for (i = 0; i < sizeofty_in_dwords; i++) {
      if (classes[i] == X86_64_MemoryClass) {
        return 0;
      }
    }
...
void ParmLocator::LocateNextParm(MIRType *ty, PLocInfo &ploc) {
  ...
  switch (ty->GetPrimType()) {
    ...
    case PTY_ref:
      int_parm_num_++;
      if (int_parm_num_ <= X86Abi::num_int_parm_regs) {
        ploc.reg0 = X86Abi::int_parm_regs[int_parm_num_ - 1];
      }
      break;
    ...
    case PTY_f64:
      float_parm_num_++;
      if (float_parm_num_ <= X86Abi::num_float_parm_regs) {
        ploc.reg0 = X86Abi::float_parm_regs[float_parm_num_ - 1];
      }
      break;
    ...
    case PTY_c128:
      ...
      if (float_parm_num_ <= X86Abi::num_float_parm_regs) {
        ...
        float_parm_num_++;
        if (float_parm_num_ <= X86Abi::num_float_parm_regs) {
          ploc.reg1 = X86Abi::float_parm_regs[float_parm_num_ - 1];
    ...
    case PTY_agg: {
      X86_64_ArgumentClass classes[2];
      int32 save_int_parm_num = int_parm_num_;
      int32 save_float_parm_num = float_parm_num_;
      ...
      if (numregs > 0) {  // passing in registers
        if (classes[0] == X86_64_SseClass) {
          float_parm_num_++;
          if (float_parm_num_ <= X86Abi::num_float_parm_regs) {
            ploc.reg0 = X86Abi::float_parm_regs[float_parm_num_ - 1];
          } else {  // pass in memory
            float_parm_num_ = save_float_parm_num;
            numregs = 0;
          }
        } else {
          int_parm_num_++;
          if (int_parm_num_ <= X86Abi::num_int_parm_regs) {
            ...
            int_parm_num_ = save_int_parm_num;
            numregs = 0;
          }
        }
        if (numregs > 1) {
          if (classes[1] == X86_64_SseClass) {
            float_parm_num_++;
            if (float_parm_num_ <= X86Abi::num_float_parm_regs) {
              ploc.reg1 = X86Abi::float_parm_regs[float_parm_num_ - 1];
            } else {  // pass in memory
          ...
          } else if (classes[1] == X86_64_IntegerClass) {
            int_parm_num_++;
            if (int_parm_num_ <= X86Abi::num_int_parm_regs) {
              ploc.reg1 = X86Abi::int_parm_regs[int_parm_num_ - 1];
            } else {  // pass in memory
              float_parm_num_ = save_float_parm_num;
              int_parm_num_ = save_int_parm_num;
              ploc.reg0 = RCC;
...
ReturnMechanism::ReturnMechanism(MIRType *retty, BECommon &be) : regcount(0), fake_first_parm(false) {
  switch (retty->GetPrimType()) {
    ...
    case PTY_f64:
      regcount = 1;
      ptype0 = retty->GetPrimType();
      reg0 = X86Abi::float_return_regs[0];
      return;
    ...
    case PTY_c64:
      ...
      ptype1 = PTY_f32;
      reg0 = X86Abi::float_return_regs[0];
      reg1 = X86Abi::float_return_regs[1];
      return;
    ...
    case PTY_agg: {
      ...
      if (regcount == 0) {
        ...
        X86register next_int_return_reg = X86Abi::int_return_regs[0];
        X86register next_float_return_reg = X86Abi::float_return_regs[0];
        if (classes[0] == X86_64_IntegerClass) {
          reg0 = X86Abi::int_return_regs[0];
          ptype0 = PTY_i64;
          next_int_return_reg = X86Abi::int_return_regs[0];
        } else {
          reg0 = X86Abi::float_return_regs[0];
          ptype0 = PTY_f64;
          next_float_return_reg = X86Abi::float_return_regs[1];
        }
        if (regcount > 1) {
          if (classes[0] == X86_64_IntegerClass) {
