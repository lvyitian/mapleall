...
#undef OPTIMIZE_FOR_PROLOG // riscv has no load/store double, evaluate this opt later
...
class LocalRaInfo {
 public:
  ...
  //uint64 globalPregMask;  // global phys reg used in bb
  //uint64 localPregMask;   // local phys reg used in bb

  explicit LocalRaInfo(MapleAllocator *mallocator)
      : allocator(mallocator),
        defCnt(mallocator->Adapter()),
        useCnt(mallocator->Adapter()) {}
        //globalPreg(mallocator->Adapter()),
        //globalPregMask(0),
        //localPregMask(0) {}
};
...
#define FOREACH_LRA_REGS(isInt, regno, localRa)                                        \
  {                                                                                    \
    for ((regno) = ((isInt) ? R0 : V0); (regno) <= ((isInt) ? R31 : V31); (regno)++) { \
...
class LocalRegAllocator {
 public:
  ...
  void InitRegs(uint32 intMax, uint32 fpMax, bool hasYield) {
    // exclude 0th bit, no reserved regs like sp/fp
    intRegs = 0x1fffffdc0ULL;
#ifdef RESERVED_REGS
    fpRegs = 0x0fffffffeULL;
#else
    fpRegs = 0x1fffffffeULL;
#endif
  }
};
