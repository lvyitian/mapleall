...
#define MAX_INT_SPILL 2
#define MAX_FP_SPILL 2

namespace maplebe {

class Riscv64RegAllocator : public RegAllocator {
 protected:
  bool avail_reg_set_[MAXREG];
  MapleMap<uint32, Riscv64reg_t> reg_map_;  // virtual-register-to-physical-register map
  ...
  Riscv64reg_t atomic_store_result_reg;
 ...
 public:
  Riscv64RegAllocator(CGFunc *cgfunc, MapleAllocator *mallocator)
    : RegAllocator(cgfunc),
  ...
  virtual ~Riscv64RegAllocator() {}

  void InitAvailReg();
  bool AllocatePhysicalRegister(RegOperand *opnd, OpndProp *prop);
  void PreAllocate();
  void ReleaseReg(RegType regty, Riscv64reg_t reg);
  void ReleaseReg(RegOperand *regopnd, OpndProp *prop);
  void GetPhysicalRegisterBank(RegType regty, uint8 &start, uint8 &end);
  void AllocHandleCallee(Insn *insn, const Riscv64MD *md);
  bool IsSpecialReg(Riscv64reg_t reg);
  bool IsUntouchableReg(uint32 regno);
  void SaveCalleeSavedReg(RegOperand *opnd);
  virtual void StorePseudoRegister(RegOperand *regopnd, Riscv64reg_t regNo, Insn *insn, BB *bb) {}
...
class DefaultO0RegAllocator : public Riscv64RegAllocator {
 public:
  DefaultO0RegAllocator(CGFunc *cgfunc, MapleAllocator *mallocator) : Riscv64RegAllocator(cgfunc, mallocator) {}
...
class O1RegAllocator : public Riscv64RegAllocator {
 public:
  O1RegAllocator(CGFunc *cgfunc, MapleAllocator *mallocator)
    : Riscv64RegAllocator(cgfunc, mallocator), m_intRegIndex(0), m_floatRegIndex(0), m_validIndex(0) {}

  ~O1RegAllocator() override {}

  bool AllocateRegisters() override;
  regno_t DoRegisterSpill(RegOperand *regopnd, Insn *insn, bool isSrcRegister, BB *bb) override;
  void StorePseudoRegister(RegOperand *regopnd, Riscv64reg_t regNo, Insn *insn, BB *bb) override;
  Operand *GetOperandFromAllocatedSet(regno_t);
  regno_t GetVirtualRegFromPhysicalReg(Riscv64reg_t);
...
class Riscv64CallerSavedRegHandler : public CallerSavedRegHandler {
 public:
  Riscv64CallerSavedRegHandler(CGFunc *f /*, MapleAllocator* mallocator*/) : CallerSavedRegHandler(f) {}

  ~Riscv64CallerSavedRegHandler() {}

  inline static int Reg2BitPos(Riscv64reg_t r) {
    return int(r) - 1;
  }

  inline static Riscv64reg_t BitPos2Reg(int p) {
    return Riscv64reg_t(p + 1);
  }
};

static regno_t kFirstIntParamReg = R10;
static regno_t kLastIntParamReg = R17;
static regno_t kFirstIntParamEnum = kFirstIntParamReg - R0;
static regno_t kLastIntParamEnum = kLastIntParamReg - R0;

static regno_t kFirstFpParamReg = V10;
static regno_t kLastFpParamReg = V17;
static regno_t kFirstFpParamEnum = kFirstFpParamReg - R0;
static regno_t kLastFpParamEnum = kLastFpParamReg - R0;

static regno_t kFirstFpParamRegNum = kFirstFpParamReg - V0;
static regno_t kLastFpParamRegNum = kLastFpParamReg - V0;

static regno_t kSpillRegEnum0 = 30;
static regno_t kSpillRegEnum1 = 31;

class LSRALinearScanRegAllocator : public Riscv64RegAllocator {
  enum SimdSlot {
    /*
 ...
 public:
  LSRALinearScanRegAllocator(CGFunc *cgfunc, MapleAllocator *mallocator)
    : Riscv64RegAllocator(cgfunc, mallocator),
