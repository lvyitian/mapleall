...
class PtrList {
 public:
  PtrList() : first(nullptr), last(nullptr) {}
  explicit PtrList(T* ptr) : first(ptr), last(ptr) {}
  PtrList(T* ptr1, T* ptr2) : first(ptr1), last(ptr2 == nullptr ? ptr1 : ptr2) {}
  class iterator {
   public:
    iterator(T *p) : ptr(p) {};
    void operator++() {
  ...
  const iterator end() const {
    return iterator(nullptr); // assuming last->GetNext() is always nullptr
  }
  ...
  void insert(iterator iter, T *ptr) {
    T *tmp = *iter;
    ASSERT(tmp, "empty iterator, push_back?");
    if (tmp) {
      T *prev = tmp->GetPrev();
      ptr->SetPrev(prev);
      ptr->SetNext(tmp);
  ...
  void insertAfter(iterator iter, T *ptr) {
    T *tmp = *iter;
    ASSERT(tmp, "empty iterator");
    if (tmp) {
      T *next = tmp->GetNext();
      ptr->SetNext(next);
      ptr->SetPrev(tmp);
  ...
  void push_back(T *ptr) {
    ...
    ASSERT(ptr, "pushing back nullptr");
    ptr->SetPrev(last);
    ptr->SetNext(nullptr);
    last = ptr;
  }
  void update_front(T *ptr) {
    if (ptr) {
      ptr->SetPrev(nullptr);
    }
    first = ptr;
  }
  void update_back(T *ptr) {
    if (ptr) {
      ptr->SetNext(nullptr);
    }
    last = ptr;
...
class PtrListRef {
 public:
  ...
  void verify() {
    for (T *item = first; item; ) {
      T *next = item->GetNext();
      ASSERT(next == nullptr || next->GetPrev() == item, "List broken");
      item = next;
    }
  }
