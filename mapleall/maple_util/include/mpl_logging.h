...
/***************************************************************************
   ...
   LOG(LT_MPLME, "deleting BB %d because it is unreachable", bb->id.idx);
   ...
   EmitLogForDevelop() - for DBG and LOG
...
#if DEBUG
constexpr bool g_enableDebug = true;
#define DEBUG_TEST 1
#else
constexpr bool g_enableDebug = false;
#define DEBUG_TEST 0
#endif

const unsigned int kMaxLogLen = 2048;
...
// for developer
#define PRINT_LEVEL_DEV maple::kLlLog

#define DBG(tag, fmt, ...)                                                                      \
  do {                                                                                          \
    if (PRINT_LEVEL_DEV <= maple::kLlLog)                                                              \
      maple::logInfo.EmitLogForDevelop(tag, maple::kLlLog, __FILE__, __FUNCTION__, __LINE__, fmt, ##__VA_ARGS__); \
  } while (0)
#define LOG(tag, fmt, ...)                                                                      \
  do {                                                                                          \
    if (PRINT_LEVEL_DEV <= maple::kLlLog)                                                              \
...
#define DCHECK(cond, fmt, ...)         \
  do {                                 \
    if (maple::g_enableDebug)                 \
      CHECK(cond, fmt, ##__VA_ARGS__); \
  } while (0)
...
// for user
#define PRINT_LEVEL_USER maple::kLlInfo

#define INFO(num, fmt, ...)                                     \
  do {                                                          \
    if (PRINT_LEVEL_USER <= maple::kLlInfo)                            \
      maple::logInfo.EmitLogForUser(num, maple::kLlInfo, fmt, ##__VA_ARGS__); \
  } while (0)
...
#define WARN(num, fmt, ...)                                     \
  do {                                                          \
    if (PRINT_LEVEL_USER <= maple::kLlWarn)                            \
      maple::logInfo.EmitLogForUser(num, maple::kLlWarn, fmt, ##__VA_ARGS__); \
  } while (0)
#define ERR(num, fmt, ...)                                     \
  do {                                                         \
    if (PRINT_LEVEL_USER <= maple::kLlErr)                            \
      maple::logInfo.EmitLogForUser(num, maple::kLlErr, fmt, ##__VA_ARGS__); \
  } while (0)
#define FATAL(num, fmt, ...)                                     \
  do {                                                           \
    if (PRINT_LEVEL_USER <= maple::kLlFatal)                            \
      maple::logInfo.EmitLogForUser(num, maple::kLlFatal, fmt, ##__VA_ARGS__); \
    abort();                                                     \
  } while (0)
