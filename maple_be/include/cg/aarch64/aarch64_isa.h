...
/*
   ...
   D1.8.2
 */
#define AARCH64_STACK_PTR_ALIGNMENT 16
...
#define STP_LDP_IMM64_LOWER_BOUND -512
#define STP_LDP_IMM64_UPPER_BOUND 504
#define STP_LDP_IMM32_LOWER_BOUND -256
#define STP_LDP_IMM32_UPPER_BOUND 252

#define STR_LDR_PRE_POST_LOWER_BOUND -256
#define STR_LDR_PRE_POST_UPPER_BOUND 255
#define STRALL_LDRALL_IMM_LOWER_BOUND 0
#define STR_LDR_IMM32_UPPER_BOUND 16380 /* must be a multiple of 4 */
#define STR_LDR_IMM64_UPPER_BOUND 32760 /* must be a multiple of 8 */
...
static inline bool IsGPRegister(AArch64reg_t r) {
  return (R0 <= r && r <= RZR);
}

static inline bool IsFPSIMDRegister(AArch64reg_t r) {
  return (V0 <= r && r <= V31);
}
...
static inline RegType GetRegType(AArch64reg_t r) {
  if (IsGPRegister(r)) {
    return kRegTyInt;
...
struct RegProp {
  ...
  AArch64reg_t physical_reg_;
  uint32 def_use_;  // used for register use/define and other properties of other operand
  RegProp(RegType t, SubRegType subt, AArch64reg_t r, uint32 d) : regtype_(t),
      subRegType(subt), physical_reg_(r), def_use_(d) {}
};

struct AArch64OpndProp : public OpndProp {
  Operand::OperandType opnd_ty_;
  RegProp regprop_;
  uint8 size_;

  AArch64OpndProp(Operand::OperandType t, RegProp p, uint8 s) : opnd_ty_(t), regprop_(p), size_(s) {}
...
struct AArch64MD {
  ...
  inline AArch64OpndProp *GetOperand(int nth) const {
    CG_ASSERT(nth < 5, "");
    return static_cast<AArch64OpndProp *>(operand_[nth]);
  }
